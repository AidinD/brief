 ...
 *
 * What stays here is the middle: which guards Brief wants, and how Brief builds.
 *
...
// Brief publishes from here, so there is no tag to guard - electron-builder
...
// `dist/` cannot be cleared while a packaged Brief holds a file in it, and the
// app test harness leaves exactly such a process behind. Matched on the
// executable path, so an installed Brief and every other Electron app are left
// alone.
